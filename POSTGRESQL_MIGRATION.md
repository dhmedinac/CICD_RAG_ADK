...
# Format: postgresql+cloudsql://user:password@PROJECT_ID%3AREGION%3AINSTANCE/database
...
DB_URL="postgresql+cloudsql://${DB_USER}:${DB_PASSWORD}@${CONNECTION_NAME_ENCODED}/rag_agent_sessions"
...
- Uses the Cloud SQL Python Connector format: `postgresql+cloudsql://user:password@PROJECT:REGION:INSTANCE/database`
...
- Ensure DATABASE_URL uses the correct format: `postgresql+cloudsql://user:password@PROJECT:REGION:INSTANCE/database`
...
- Ensure you're using `postgresql+cloudsql://` scheme (not just `postgresql+asyncpg://`)
- **Critical:** Colons in the connection name MUST be URL-encoded as `%3A`
  - ✅ Correct: `postgresql+cloudsql://user:password@PROJECT_ID%3AREGION%3AINSTANCE/database`
  - ❌ Wrong: `postgresql+cloudsql://user:password@PROJECT_ID:REGION:INSTANCE/database`
...
| Connection (Cloud Run) | N/A | `postgresql+cloudsql://user:password@PROJECT_ID%3AREGION%3AINSTANCE/database` |
